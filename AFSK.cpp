...
SPISettings SPIDAC(2000000, MSBFIRST, SPI_MODE0);
...
WRITE_PERI_REG( PERIPHS_GPIO_BASEADDR + 8, 0x8000 );
 SPI.beginTransaction (MCP3008);
  ...
  WRITE_PERI_REG( PERIPHS_GPIO_BASEADDR + 4, 0x8000 );
  SPI.endTransaction   ();
...
void dacanalogWrite(uint16_t value)
{
    ...
    // Assert SPI bus
    WRITE_PERI_REG( PERIPHS_GPIO_BASEADDR + 8, 0x01 );
    ...
    // Deassert SPI bus
    WRITE_PERI_REG( PERIPHS_GPIO_BASEADDR + 4, 0x01 );
...
SPI.begin     ();
  ...
  timer1_write(8333.333333333333); //9600
...
dacanalogWrite(sinwave);

  }else{
adcVal =  adc_single_channel_read (adc_single_ch1);
  
  AFSK_adc_isr(AFSK_modem, ((int16_t )((adcVal) >> 2) - 128));
//dacanalogWrite(0);
digitalWrite(D4, LOW);
